namespace Ecommittees.Model
{
  ...
  //      {
		//	IStorageItem inputFile = await ApplicationData.Current.LocalFolder.TryGetItemAsync(PlainSha256 + ".ano");
		//	if (inputFile != null)
		//	{
		//		return (await FileIO.ReadBufferAsync((StorageFile)inputFile)).ToArray();
		//	}
		...
		//		inputFile = await ApplicationData.Current.LocalFolder.GetFileAsync(PlainSha256);
		//		byte[] ibuff = (await FileIO.ReadBufferAsync((StorageFile)inputFile)).ToArray();
